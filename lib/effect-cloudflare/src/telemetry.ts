...
const readOtelConfig = Effect.gen(function* () {
  ...
  const ingestKey = yield* Config.option(
    Config.redacted("MAPLE_OTEL_INGEST_KEY"),
  )
...
const loggingFetch: typeof globalThis.fetch = async (input, init) => {
  ...
  try {
    ...
    if (res.ok) {
      console.log(
        `[telemetry] ${init?.method ?? "GET"} ${url} → ${res.status} (${ms}ms)`,
      )
    } else {
...
export const makeTelemetryLayer = (
  ...
  Layer.unwrap(
    Effect.gen(function* () {
      ...
      const interval: Duration.Input | undefined = exportOnShutdownOnly
        ? "1 hour"
        : undefined
...
export const logTelemetryConfigOnce = (env: Record<string, unknown>) => {
  ...
  configLogged = true
  const has = (k: string) =>
    env[k] != null && String(env[k]) !== "" ? "set" : "MISSING"
  console.log(
