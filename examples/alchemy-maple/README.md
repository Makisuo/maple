...
| [`alchemy.run.ts`](alchemy.run.ts) | A Discord destination, two alert rules, a dashboard, a scoped API key        |
...
```bash
MAPLE_API_KEY=maple_ak_… CLOUDFLARE_ACCOUNT_ID=… DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/… bun alchemy deploy
```
