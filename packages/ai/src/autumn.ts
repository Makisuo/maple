...
const postTrack = async (
	...
	try {
		...
		if (!response.ok) {
			...
			console.warn(
				`[ai-billing] track failed: ${response.status} feature=${event.featureId} body=${body}`,
			)
		}
	} catch (error) {
		console.warn(
			`[ai-billing] track error feature=${event.featureId}: ${error instanceof Error ? error.message : String(error)}`,
		)
	}
}
