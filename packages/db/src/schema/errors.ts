...
export const errorIssues = sqliteTable(
  ...
  (table) => [
    ...
    index("error_issues_org_assignee_idx").on(
      table.orgId,
...
export const errorIssueEvents = sqliteTable(
  ...
  (table) => [
    index("error_issue_events_issue_idx").on(
      table.orgId,
      table.issueId,
      table.createdAt,
    ),
    index("error_issue_events_actor_idx").on(
      table.orgId,
      table.actorId,
      table.createdAt,
    ),
    index("error_issue_events_type_idx").on(
      table.orgId,
      table.type,
      table.createdAt,
    ),
...
export const errorNotificationPolicies = sqliteTable(
  "error_notification_policies",
  {
    ...
    notifyOnTransitionDone: integer("notify_on_transition_done", {
      ...
      .notNull()
      .default(0),
    notifyOnClaim: integer("notify_on_claim", { mode: "number" })
      .notNull()
      .default(0),
