...
import { agentTraceFacetsQuery, agentTraceListQuery, agentTraceSummaryQuery, agentTraceTimelineQuery } from "./genai"

const WINDOW = { startTime: "2026-08-01 00:00:00", endTime: "2026-08-05 00:00:00" }
const listParams = { orgId: "org_1", ...WINDOW }
const agentTraceParams = { ...listParams, agentTraceId: "conv_42" }
...
describe("agent trace identity", () => {
	...
	it("coalesces conversation id → session id → trace id, per trace", () => {
		const { sql } = compileCH(agentTraceTimelineQuery(), agentTraceParams)
		expect(sql).toContain("OVER (PARTITION BY TraceId)")
		expect(sql).toContain("multiIf(max(if(SpanAttributes['gen_ai.conversation.id']")
		// TraceId is the last resort, so a single-turn trace is still an agent trace.
		expect(sql).toMatch(/TraceId\) AS agentTraceId/)
	})
	...
	it("ranks the identity ladder conversation.id → session.id → TraceId", () => {
		...
		const agentTraceIdExpr = sql.slice(start, sql.indexOf(" AS agentTraceId", start))
		const conversation = agentTraceIdExpr.indexOf("gen_ai.conversation.id")
		const session = agentTraceIdExpr.indexOf("session.id")
		const traceId = agentTraceIdExpr.lastIndexOf("TraceId)")
...
describe("agentTraceTimelineQuery", () => {
	it("scopes to the org, the window, and GenAI spans only", () => {
		const compiled = compileCH(agentTraceTimelineQuery(), agentTraceParams)
	...
	it("pins one agent trace and returns spans in chronological order", () => {
		const { sql } = compileCH(agentTraceTimelineQuery(), agentTraceParams)
		expect(sql).toContain("WHERE agentTraceId = 'conv_42'")
...
describe("agentTraceListQuery", () => {
	it("groups by the derived agent trace id", () => {
		const { sql } = compileCH(agentTraceListQuery(), listParams)
		expect(sql).toContain("GROUP BY agentTraceId")
	...
	it("filters agent traces post-aggregation so totals stay complete", () => {
		const { sql } = compileCH(agentTraceListQuery({ model: "gpt-4o", hasTools: true }), listParams)
		// A pre-aggregation model filter would drop the agent trace's other-model
	...
	it("treats error as outranking running, so the three statuses partition the list", () => {
		expect(compileCH(agentTraceListQuery({ status: "error" }), listParams).sql).toContain("errorCount > 0")
		const running = compileCH(agentTraceListQuery({ status: "running" }), listParams).sql
		...
		// the others — an agent trace that errored is not ok, and neither is one still
		// running. Without this the three buckets overlap and the counts double.
		const ok = compileCH(agentTraceListQuery({ status: "ok" }), listParams).sql
	...
	it("excludes cost-less agent traces from a cost bound", () => {
		const { sql } = compileCH(agentTraceListQuery({ costMin: 0.01 }), listParams)
		expect(sql).toContain("cost >= 0.01")
		expect(sql).toContain("costSpanCount > 0")
	})

	it("sorts by the requested dimension", () => {
		expect(compileCH(agentTraceListQuery({ sort: "cost" }), listParams).sql).toContain("ORDER BY cost DESC")
		expect(
			compileCH(agentTraceListQuery({ sort: "turns", sortDirection: "asc" }), listParams).sql,
...
describe("agentTraceSummaryQuery", () => {
	// The header and the list row must never disagree about the same agent trace,
	// which is why they are the same aggregate with one extra predicate.
	it("is the list aggregate pinned to one agent trace", () => {
		const summary = compileCH(agentTraceSummaryQuery(), agentTraceParams).sql
		const list = compileCH(agentTraceListQuery(), listParams).sql
		expect(summary).toContain("HAVING agentTraceId = 'conv_42'")
	...
	it("scopes to the org", () => {
		const compiled = compileCH(agentTraceSummaryQuery(), agentTraceParams)
		expect(compiled.tenantScope).toBe("org")
		expect(compiled.sql).toContain("OrgId = 'org_1'")
	})
})

describe("agentTraceFacetsQuery", () => {
	it("emits every sidebar dimension with the shared {name, count, facetType} shape", () => {
		const { sql } = compileUnion(agentTraceFacetsQuery(), listParams)
	...
	// An agent trace that used three models contributes one count to each of them.
	it("explodes multi-valued dimensions and counts distinct agent traces", () => {
		const { sql } = compileUnion(agentTraceFacetsQuery(), listParams)
		expect(sql).toContain("arrayJoin(models) AS name")
		expect(sql).toContain("uniq(agentTraceId) AS count")
	})
	...
	it.each(FACET_BRANCHES)("the %s branch drops its own predicate", (_name, marker, ownPredicate) => {
		const { sql } = compileUnion(agentTraceFacetsQuery(ALL_FILTERS), listParams)
	...
	// Every branch re-runs the whole agent trace aggregate, so the eight numeric
	...
	it("keeps every branch org-scoped", () => {
		expect(compileUnion(agentTraceFacetsQuery(), listParams).tenantScope).toBe("org")
	})
})
