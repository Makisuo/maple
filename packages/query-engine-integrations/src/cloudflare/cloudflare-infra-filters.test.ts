...
describe("cloudflareFilterConditions", () => {
	...
	it("routes the host filter through the same transitional coalesce as the grouping key", () => {
		...
		const { sql } = compileCHUnsafe(
			cloudflareZoneFirewallTopSQL({ hosts: ["a.example.com"] }),
			zoneParams,
		)
	...
	it("produces no condition for empty or absent values", () => {
		const bare = compileCHUnsafe(cloudflareZoneBreakdownTotalsSQL("path"), zoneParams).sql
		const empty = compileCHUnsafe(
