/**
 * Migration 0015 — AI classification columns on `traces`.
 *
 ...
 * therefore has to become "15" so the routing gate catches that org first.
 *
 ...
 * cluster still at 14 resolves `clickhouse_ready = false` and its traffic routes
 * to the managed pipeline instead of its own cluster. No data is lost and no
 * insert fails; the org silently falls back until `applySchema` stamps 15, at
 * which point routing returns on the next 30s cache TTL. This is the designed
 * mechanism, not a side effect — contrast migrations 0010/0014, which changed
 * nothing the gateway writes and so stayed `requiredForIngest: false`.
 *
 ...
 * statements backfill clusters already at version 14. Deliberately no
 ...
 * rules.
 */
export const migration_0015_ai_classification_columns = {
	version: 15,
