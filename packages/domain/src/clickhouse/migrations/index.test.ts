...
import { migration_0023_ai_trace_index } from "./0023_ai_trace_index"
...
describe("ClickHouse migrations", () => {
	it("keeps migrations ordered by version", () => {
		...
		expect(migrations.at(-1)).toBe(migration_0023_ai_trace_index)
		...
		// before ingest routes there again. 0022 and 0023 are read-path only
		// again: every table they touch is MV-populated and the gateway writes
		// none of them.
		...
		expect(migration_0023_ai_trace_index.requiredForIngest).toBe(false)
	})
