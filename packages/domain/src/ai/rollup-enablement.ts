 ...
 *
 * The boundary is **the hour the classifying gateway binary reaches the whole
 * fleet** (classification is unconditional — there is no flag), not the hour
 * the MV was created (under a pre-classification binary `WHERE AiVendor != ''`
 * matches nothing, so MV creation truncates an empty hour). That hour is partly
 * classified and **indistinguishable from a complete one**: every counter in it
 * is internally consistent, `EligibleSpanCount` still equals the sum of the
 * four state counters, and nothing in the row says most of the hour never
 ...
 * runbook, spelled out in migration 0017's header. A rollback to a
 * pre-classification binary and a re-deploy during the migration window records
 * a *fresh* boundary, and the hours between the two are unclassified and fall
 * under the same rule. Post-rollout the boundary never moves again.
 */
