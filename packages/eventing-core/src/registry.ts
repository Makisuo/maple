...
export class ProjectorRegistry {
	...
	register<TConfig>(projector: SignalProjector<TConfig>): this {
		...
		if (projector.outputType.length === 0) throw new Error("projector output type must not be empty")
		if (projector.dataSchema.length === 0) throw new Error("projector data schema must not be empty")
