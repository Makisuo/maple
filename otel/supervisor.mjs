...
const CONFIG_PATH = process.env.COLLECTOR_CONFIG_PATH ?? "/etc/otelcol/config.yaml"
const OTELCOL_BIN = process.env.OTELCOL_BIN ?? "/usr/local/bin/otelcol-contrib"
...
const RELOAD_PORT = Number(process.env.PORT ?? process.env.COLLECTOR_RELOAD_PORT ?? "13140")
const RESTART_TIMEOUT_MS = Number(process.env.COLLECTOR_RESTART_TIMEOUT_MS ?? "30000")
const MAX_BODY_BYTES = Number(process.env.COLLECTOR_MAX_CONFIG_BYTES ?? "2000000")
...
const looksLikeCollectorConfig = (yaml) =>
	/^\s*(receivers|exporters|processors|service|connectors|extensions)\s*:/m.test(yaml)
		...
		if (req.method === "PUT" && req.url === "/-/reload") {
			...
			try {
				...
				res.writeHead(413, { "content-type": "text/plain" }).end(
					error.message ?? "request body error",
				)
				return
			}
