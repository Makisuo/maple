...
import { FilterSection, SearchableFilterSection, type FilterOption } from "@/components/filters/filter-section"
import {
	FilterSidebarBody,
	FilterSidebarError,
	FilterSidebarFrame,
...
import { Route } from "@/routes/agent-traces"
import { NumericRangeFilterSection } from "./numeric-range-filter-section"
import { formatTokens } from "./agent-traces-format"
import { COST_OUTLIER_DOLLARS } from "./agent-traces-filter-inputs"

interface AgentTraceFacetItem {
	readonly name: string
	readonly count: number
}

export interface AgentTraceFacets {
	readonly models: ReadonlyArray<AgentTraceFacetItem>
	readonly providers: ReadonlyArray<AgentTraceFacetItem>
	readonly agents: ReadonlyArray<AgentTraceFacetItem>
	readonly workflows: ReadonlyArray<AgentTraceFacetItem>
	readonly services: ReadonlyArray<AgentTraceFacetItem>
	readonly finishReasons: ReadonlyArray<AgentTraceFacetItem>
	readonly statuses: ReadonlyArray<AgentTraceFacetItem>
	readonly toolCount: number
	readonly redactedCount: number
	/** `name` is the bucket floor in ms; ceilings are floor × √2. */
	readonly durationBuckets: ReadonlyArray<AgentTraceFacetItem>
...
/**
 * Share of agent traces the duration axis must cover before the rest folds into a
...
export function toDurationBuckets(raw: ReadonlyArray<AgentTraceFacetItem>): RangeBucket[] {
...
function withSelected(options: ReadonlyArray<AgentTraceFacetItem>, selected?: string): FilterOption[] {
...
interface AgentTracesFilterSidebarProps {
	facetsResult: Result.Result<AgentTraceFacets, unknown>
}

export function AgentTracesFilterSidebar({ facetsResult }: AgentTracesFilterSidebarProps) {
		...
		.onSuccess((facets, result) => {
			...
			return (
				<FilterSidebarFrame waiting={result.waiting}>
					...
					<FilterSidebarBody>
						{/* The four one-click slices that used to sit beside the search
						    box. They write the same params the sections below do, so a
						    chip and its section can never disagree. */}
						<QuickFilters
							erroredCount={facets.statuses.find((item) => item.name === "error")?.count ?? 0}
							truncatedCount={
								facets.finishReasons.find((item) => item.name === "length")?.count ?? 0
							}
							toolCount={facets.toolCount}
							...
							onToggle={(patch) => navigate({ search: (prev) => ({ ...prev, ...patch }) })}
						/>
						...
						{/* An agent trace counts under every model it used, not only the
						...
						<RangeFilterSection
							...
							histogramUnitLabel="agent traces"
							defaultOpen
						/>
						...
						{!hasFacets && (
							<p className="py-4 text-sm text-muted-foreground">
								No agent traces in the selected time range
							</p>
						)}
					</FilterSidebarBody>
				</FilterSidebarFrame>
...
/** The URL patch a chip writes when clicked. */
type QuickFilterPatch = Record<string, string | number | boolean | undefined>
...
interface QuickFiltersProps {
	...
	usedTools: boolean
	onToggle: (patch: QuickFilterPatch) => void
}

/**
 * Triage in one click: the four slices anyone opening this page is most likely
 * to want. Each is a shortcut into a section further down the sidebar — errored
 * is Status, truncated is Finish reason, over-$1 is the Cost range's floor — so
 * toggling either surface leaves the other showing the same thing.
 */
	...
	const chips = [
		{
			...
			title: "Agent traces that ended in an error",
			patch: { status: statusError ? undefined : "error" },
		},
		{
			key: "truncated",
			label: "Truncated",
			...
			title: "Agent traces whose response hit the token cap — a silent failure",
			patch: { finishReason: truncatedOnly ? undefined : "length" },
		},
		{
			key: "expensive",
			label: `Over $${COST_OUTLIER_DOLLARS.toFixed(2)}`,
			active: expensiveOnly,
			title: `Agent traces that cost more than $${COST_OUTLIER_DOLLARS.toFixed(2)}`,
			patch: { costMin: expensiveOnly ? undefined : COST_OUTLIER_DOLLARS },
		},
		{
			key: "tools",
			label: "Used tools",
			count: toolCount,
			active: usedTools,
			title: "Agent traces that executed at least one tool",
			patch: { hasTools: usedTools ? undefined : true },
		},
	]

	return (
		<div className="flex flex-col gap-2 py-2">
			<span className={`${FILTER_SECTION_LABEL} text-muted-foreground`}>Quick filters</span>
			<div className="flex flex-wrap gap-1.5">
				{chips.map((chip) => (
					<button
						...
						title={chip.title}
						className={cn(
							"inline-flex h-6.5 items-center gap-1.5 rounded-full border px-2.25 font-mono text-xs transition-colors",
							chip.active
...
interface AgentTraceIdFilterProps {
...
function AgentTraceIdFilter({ value, onApply }: AgentTraceIdFilterProps) {
	...
	return (
		...
		>
			<Label
				htmlFor="agent-traces-id-filter"
			...
			<InputGroup>
				...
				{text && (
					<InputGroupAddon align="inline-end">
						<InputGroupButton
							aria-label="Clear agent trace id filter"
							onClick={() => {
								setText("")
								onApply(undefined)
							}}
