...
import { formatCost, formatTokens } from "./agent-trace-format"
import type { AgentTraceModel, AgentTraceSummaryData } from "./agent-trace-model"

// ---------------------------------------------------------------------------
// The context rail: a scrubbable map of the whole agent trace, then who spent what.
//
...
export function AgentTraceRail({
	...
	return (
		// The rail owns its own inset: `PageLayout.RightSidebar` supplies the border
		// and the width but no padding, so without this every figure sits flush
		// against the divider.
		<div className="flex flex-col gap-6 px-6 pt-7 pb-8">
			<AgentTraceMap
				model={model}
...
function TokenBar({ tokens }: { tokens: AgentTraceModel["tokens"] }) {
...
/**
 * The agent trace map: one bar per event, length proportional to its duration,
 ...
 * to that event. On a 142-turn agent trace this is the only view of the whole
 * shape.
 */
function AgentTraceMap({
	model,
	visibleRowIds,
	onSelectRow,
	live,
	...
	return (
		<section className="flex flex-col gap-2.5">
			<div className="flex items-baseline gap-2">
				<h3 className={cn(LABEL, "grow")}>Agent trace map</h3>
				<span className={VALUE}>
			...
			<div
				ref={listRef}
				role="group"
				aria-label="Agent trace map — arrow keys move between events"
				onKeyDown={onKeyDown}
