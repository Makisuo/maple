...
import { formatTokensCompact, humaniseFinishReason } from "./agent-trace-format"
...
export type AgentTraceBadgeTone = "neutral" | "amber" | "red" | "live"

const toneClasses: Record<AgentTraceBadgeTone, string> = {
...
export function AgentTraceBadge({
	tone = "neutral",
	icon,
	children,
	title,
 ...
 */
export function AgentTraceEventBadges({ event }: { event: AgentTraceEventData }) {
	...
	if (event.reasoningTokens != null && event.reasoningTokens > 0) {
		badges.push(
			<AgentTraceBadge key="reasoning" title="Reasoning tokens are billed but never shown">
	...
	if (finish === "length" || finish === "max_tokens") {
		...
		badges.push(
			<AgentTraceBadge key="finish" tone="amber">
	...
	if (event.status === "error") {
		badges.push(
			<AgentTraceBadge key="error" tone="red" icon={<XmarkIcon size={9} className="shrink-0" />}>
				PROVIDER ERROR
			</AgentTraceBadge>,
		)
	}
