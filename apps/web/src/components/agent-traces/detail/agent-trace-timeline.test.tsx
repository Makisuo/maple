 ...
 * the agent trace map scrolls through the second.
 */
...
function renderTimeline(events: ReadonlyArray<AgentTraceEventData>, isLive = false) {
	const model = buildAgentTraceModel(events, null)
	return render(<AgentTraceTimeline model={model} isLive={isLive} truncated={false} />)
}

/** 60 unremarkable turns — past the threshold where the middle folds into a group. */
function longAgentTrace(): ReadonlyArray<AgentTraceEventData> {
...
describe("AgentTraceTimeline", () => {
	...
	it("scales a redacted turn's timing bar against the longest turn in the agent trace", () => {
	...
	it("marks the still-open turn of a running agent trace as live", () => {
		renderTimeline(
	...
	it("offers navigation chips only once the agent trace is long enough to need them", () => {
		const short = renderTimeline([event({ id: "m1", content: "a" })])
		expect(screen.queryByLabelText("Filter this agent trace")).toBeNull()
		short.unmount()

		renderTimeline(longAgentTrace())
		expect(screen.getByLabelText("Filter this agent trace")).toBeTruthy()
	})
})

describe("AgentTraceTimeline — revealing a row the map asked for", () => {
	beforeEach(() => {
		Element.prototype.scrollIntoView = vi.fn()
	})

	function renderWithHandle(events: ReadonlyArray<AgentTraceEventData>) {
		const ref = createRef<AgentTraceTimelineHandle>()
		const model = buildAgentTraceModel(events, null)
		const view = render(<AgentTraceTimeline ref={ref} model={model} isLive={false} truncated={false} />)
		return { ref, ...view }
	}

	it("expands the collapsed group a folded row lives in, then scrolls to it", () => {
		const { ref, container } = renderWithHandle(longAgentTrace())
		// A middle turn is folded into a turn group — no node to scroll to yet.
		expect(container.querySelector('[data-agent-trace-row="m20"]')).toBeNull()

		act(() => ref.current?.revealRow("m20"))

		expect(container.querySelector('[data-agent-trace-row="m20"]')).not.toBeNull()
		expect(Element.prototype.scrollIntoView).toHaveBeenCalled()
	})

	it("drops an active filter when asked for a row that filter excludes", () => {
		const { ref, container } = renderWithHandle(longAgentTrace())
		fireEvent.change(screen.getByLabelText("Filter this agent trace"), {
...
describe("AgentTraceTimeline — reporting what is on screen", () => {
	...
	function rowNode(container: HTMLElement, id: string): Element {
		const node = container.querySelector(`[data-agent-trace-row="${id}"]`)
	...
	it("reports a collapsed group's member rows, so the map can shade the viewport", () => {
		const onVisibleRowsChange = vi.fn()
		const model = buildAgentTraceModel(longAgentTrace(), null)
		const { container } = render(
			<AgentTraceTimeline
				model={model}
				isLive={false}
		...
		const group = container.querySelector('[data-agent-trace-row^="group-"]')
