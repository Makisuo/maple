...
const tokenTone: Record<
  "default" | "signal" | "severity-warn" | "severity-crit" | "threshold",
  string
> = {
  default:
    "border-border/80 bg-background hover:border-primary/50 hover:bg-accent/40",
  signal:
  ...
  threshold:
    "border-border/80 bg-muted/40 font-mono hover:border-primary/50 hover:bg-accent/40",
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        render={
          <button
            type="button"
            className={cn(tokenBase, tokenTone.signal)}
          >
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger
        render={
          ...
          >
            <span className={cn("size-1.5 rounded-full", value === "critical" ? "bg-destructive" : "bg-severity-warn")} />
      ...
      <PopoverContent className="w-[160px] p-1" align="start">
        {(["warning", "critical"] as const).map((s) => (
          ...
          >
            <span className={cn("size-1.5 rounded-full", s === "critical" ? "bg-destructive" : "bg-severity-warn")} />
            {s}
          </button>
        ))}
      </PopoverContent>
