...
export function TraceProductEvents({
	...
	const result = useAtomValue(
		productEventsForTraceResultAtom({
				...
				<section className="rounded-md border">
					...
					<ul className="divide-y">
						{response.data.map((event) => (
							<ProductEventRow
								key={`${event.spanId}:${event.eventName}`}
								event={event}
	...
	return (
		<li>
			<button
				...
				disabled={event.spanId === ""}
				className="flex w-full flex-wrap items-center gap-x-2 gap-y-1 px-3 py-2 text-left text-xs hover:bg-muted disabled:hover:bg-transparent"
			>
