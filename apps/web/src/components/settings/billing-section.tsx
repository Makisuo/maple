				...
				{isTrialing && daysRemaining != null && trialEndsAt ? (
					<CardDescription className="space-y-1">
						...
						<span className="block text-xs text-muted-foreground">
							Your card will be charged when the trial ends. Cancel anytime before to avoid
							charges.
						</span>
					</CardDescription>
				) : (
					<CardDescription className="text-sm">Current active plan</CardDescription>
				)}
			</CardHeader>
		...
		<div className="space-y-6">
			<div className="grid grid-cols-2 gap-4">
				...
				{isLoading ? (
					...
					<UsageMeters usage={usage} limits={limits} billingPeriodLabel={billingPeriodLabel} />
				)}
			</div>
