    ...
    <div className="space-y-4">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Connect Maple to third-party services so they can act on alerts, incidents, and analytics events.
        </p>
      </header>
    ...
    >
      ...
      >
        <span
          ...
          style={{ background: "radial-gradient(circle at 30% 20%, rgba(244,111,15,0.16), transparent 70%)" }}
        />
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Forward Maple alerts into a Hazel workspace via OAuth. Once connected,
              create a Hazel destination to pick which workspace receives notifications.
            </p>
          </div>
          <a
            href={HAZEL_DOCS_URL}
        ...
        {isConnected && status ? (
          <div className="flex flex-col gap-1 rounded-md bg-muted/40 px-3 py-2 text-[11px] text-muted-foreground">
            {status.externalUserEmail ? (
              <div>
                <span className="text-foreground">{status.externalUserEmail}</span>{" "}
                authorized this connection.
              </div>
