...
import { journeyFacetsResultAtom } from "@/lib/services/atoms/warehouse-query-atoms"
...
export interface ActiveJourneyFilter {
	readonly key: string
 ...
 * this is a "you haven't sent us any journeys" state, not a "you filtered them
 * out" one, and the two need different exits.
 */
export function JourneysEmptyRange({ rangeLabel, onWiden }: { rangeLabel: string; onWiden: () => void }) {
	return (
		<EmptyFrame>
			...
			<p className="text-sm font-medium">No agent journeys in the {rangeLabel}</p>
			<p className="mt-1.5 max-w-md text-sm text-muted-foreground">
				Journeys are reconstructed from the GenAI spans your agents already emit. Nothing to configure
 ...
 *
 * The "there are N journeys in the range" line comes from a second facets read
 ...
 */
export function JourneysEmptyFilters({
	startTime,
	endTime,
	rangeLabel,
	filters,
	...
	endTime: string
	rangeLabel: string
	filters: ReadonlyArray<ActiveJourneyFilter>
	onClearAll: () => void
}) {
	const unfiltered = useAtomValue(journeyFacetsResultAtom({ data: { startTime, endTime } }))
	...
	return (
		<EmptyFrame>
			<p className="text-sm font-medium">No journeys match these filters</p>
			<p className="mt-1.5 max-w-md text-sm text-muted-foreground">
				{total != null && total > 0
					? `There are ${total.toLocaleString()} journeys in the ${rangeLabel}. Drop a filter to get back to them.`
...
/**
 * Every journey in view had its message content stripped. Said once, at the top,
 ...
 */
export function JourneyPrivacyBanner() {
	return (
		<div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-md border border-border bg-muted/40 px-3 py-2">
			...
			<p className="min-w-0 flex-1 font-mono text-xs text-muted-foreground">
				No journey in this view records message text, so journeys are named by workflow, then agent,
