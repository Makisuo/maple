...
import type { JourneyModel, JourneyRow, JourneyTimelineFilter } from "./journey-model"
import { buildTimelineView } from "./journey-model"
import type { AttachRow } from "./journey-rows"
import { JourneyTimelineRow } from "./journey-rows"

// ---------------------------------------------------------------------------
// The timeline, its toolbar, and the navigation affordances a long journey
// needs. Short journeys get two reading toggles; past the condensation
...
/**
 * What the journey map needs from the timeline. The map's bars cover every row
 ...
 */
export interface JourneyTimelineHandle {
...
export const JourneyTimeline = memo(function JourneyTimeline({
	...
	ref?: React.Ref<JourneyTimelineHandle>
}) {
	...
	// At most one row is mid-generation: on a live journey, the final assistant
	...
	const revealRow = useCallback(
		(rowId: string) => {
			...
			// Not folded away — filtered away. The map covers the whole journey, so
			...
			// Nothing left to open: the row isn't in this journey's view at all.
			pendingScrollRef.current = null
		},
		...
		<div className="flex flex-col gap-4">
					...
					<div className="flex flex-wrap items-center gap-1.5">
						...
						<span className="relative">
							...
							<Input
								...
								placeholder="Filter this journey"
								aria-label="Filter this journey"
			...
			{truncated && (
				<p className="font-mono text-[11px] text-muted-foreground">
					This journey exceeded the timeline row cap — earlier turns are not shown.
				</p>
			)}
		</div>
	)
...
function collectRowIds(rows: ReadonlyArray<JourneyRow>): ReadonlyArray<string> {
...
function scrollToRow(rowId: string): boolean {
	...
	for (const node of document.querySelectorAll<HTMLElement>("[data-journey-row]")) {
		if (node.dataset.journeyRow !== rowId) continue
...
/**
 * Reports which rows are on screen so the journey map can shade the viewport.
 *
	...
	return useCallback(
		(node: HTMLElement | null) => {
				...
				observerRef.current = new IntersectionObserver(
					(entries) => {
						for (const entry of entries) {
							const id = (entry.target as HTMLElement).dataset.journeyRow
							if (id == null) continue
			...
			return () => {
				...
				const id = node.dataset.journeyRow
