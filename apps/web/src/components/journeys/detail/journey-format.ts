...
// ---------------------------------------------------------------------------
// Journey-local formatters.
//
// Deliberately NOT `@maple/ui/lib/format`. A journey is read as a transcript,
// so the whole surface leans on two shapes the shared helpers don't produce:
//
// - clock-shaped wall time (`3m 12s`, `41m 08s`) for the journey and for the
...
// `formatJourney*` / `formatTurn*` for the same reason.
// ---------------------------------------------------------------------------

/** Wall-clock span of a journey or a collapsed run of turns: `57s`, `3m 12s`, `1h 04m`. */
export function formatJourneyDuration(ms: number | null | undefined): string {
...
export function formatTurnDuration(ms: number | null | undefined): string {
	...
	return formatJourneyDuration(ms)
}

/** Offset from the journey's first event, as it appears down the spine: `+0:00`, `+11:20`. */
export function formatJourneyOffset(ms: number): string {
 ...
 * the spine, the handoff bars, the journey map and the rail — and the same
