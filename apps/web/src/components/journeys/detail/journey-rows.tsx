...
import type {
	...
	JourneyMessageRow,
	JourneyRow,
	JourneyStructuralRow,
	JourneyTurnGroupRow,
} from "./journey-model"
import { hasContent, isInvokeAgent } from "./journey-model"
import { ContentRedactedLine, JourneyEventBadges } from "./journey-badges"
import { JourneyToolCalls } from "./journey-tool-calls"
...
export const JourneyTimelineRow = memo(function JourneyTimelineRow({
	row,
	showToolCalls,
	expandBodies,
	streaming,
	...
	attachRow?: AttachRow
	/** The longest row in the journey — the scale a redacted turn's bar reads against. */
...
function RowShell({
	...
	return (
		<div className="flex" data-journey-row={rowId} ref={attachRow}>
			<div className="flex w-14 shrink-0 justify-end pt-px pr-2.5">
				{showOffset && <span className={META}>{formatJourneyOffset(offsetMs)}</span>}
			</div>
	...
	return (
		...
		>
			<div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
				...
				<JourneyEventBadges event={event} />
				<span className="grow" />
			...
			{showToolCalls && row.toolCalls.length > 0 && <JourneyToolCalls calls={row.toolCalls} />}
		</RowShell>
	)
}
 ...
 * longest turn in the journey. A floor of 4% keeps a fast turn a bar rather than
...
function ProviderErrorPanel({ event }: { event: JourneyMessageRow["event"] }) {
...
function StructuralRow({ row, attachRow }: { row: JourneyStructuralRow; attachRow?: AttachRow }) {
	const { event } = row
...
function HandoffTargets({ event }: { event: JourneyStructuralRow["event"] }) {
...
function IdleRow({ row }: { row: JourneyIdleRow }) {
	return (
		<div className="-mt-1.5 flex">
			...
			<div className="flex min-w-0 grow items-center gap-2.5 pt-1.5 pb-4 pl-3.5">
				<span className={META}>{formatJourneyDuration(row.durationMs)} idle</span>
				<span className="text-border">·</span>
...
// ---------------------------------------------------------------------------
// Collapsed run of turns — the long-journey affordance
// ---------------------------------------------------------------------------
			...
			>
				<div className="flex flex-wrap items-center gap-x-2.5 gap-y-1">
					...
					<span className={cn(META, "flex items-baseline gap-1.5")}>
						...
						<span className="text-foreground/75">{formatJourneyDuration(row.durationMs)}</span>
						{cost != null && (
							<>
								<span className="text-border">·</span>
