...
import { hasContent } from "./journey-model"
import { JourneyBadge } from "./journey-badges"

// ---------------------------------------------------------------------------
// The overview header: identity, the five journey-level stats, and the
...
export function JourneyIdentity({
	...
	// A redacted journey has no first user message to title itself with, so it
	// falls back through workflow → lead agent → id. It must never render empty.
	const title = summary?.title?.trim() || workflow || agents[0] || journeyId
		...
		<div className="flex items-start gap-6">
			<div className="flex min-w-0 grow flex-col gap-2.5">
				<div className="flex items-center gap-2 font-mono text-[11px] leading-none">
					<span className={LABEL}>Agent journey</span>
					{workflow.length > 0 ? (
						<>
						...
						<>
							<span className="text-border">·</span>
							<span className="truncate text-muted-foreground">{journeyId}</span>
							<CopyButton
								value={journeyId}
								label="Journey ID"
								iconSize={11}
				...
				<h1 className="font-display text-[28px] leading-8 font-semibold tracking-tight text-balance text-foreground">
					{title}
				</h1>

				<div className="flex flex-wrap items-center gap-2">
					<JourneyStatusPill status={status} />
					{agents.length > 1 && (
						<JourneyBadge className="h-[22px] px-2 text-[11px] font-medium">
...
function JourneyStatusPill({ status }: { status: "ok" | "error" | "running" }) {
	const tone =
		status === "error"
			? {
					dot: "bg-destructive",
...
export function JourneyStatStrip({
	summary,
	model,
}: {
	summary: JourneySummaryData | null
	...
	return (
		<div className="flex flex-wrap items-start gap-x-6 gap-y-4 px-1">
			<Stat label="Duration" width="w-32">
				<span className="tabular-nums">{formatJourneyDuration(summary?.durationMs)}</span>
			...
			<Stat label="Models" width="w-56">
				{models.length === 0 ? (
					...
					// The multi-model case is the normal case: a journey can hop models
					...
					<span className="flex items-center gap-1.5">
						...
						{models.length > 1 && (
							<JourneyBadge title={models.join(", ")}>+{models.length - 1}</JourneyBadge>
						)}
					</span>
				)}
			</Stat>
...
export function JourneyTimeBand({ model, running }: { model: JourneyModel; running: boolean }) {
	...
	return (
		<div className="flex flex-col gap-2.5">
				...
				{(["model", "tool", "failed", "idle"] as const).map((category) => {
					...
					return (
						...
						>
							...
							/>
							{CATEGORY_STYLE[category].label} {formatJourneyDuration(total)}
						</span>
					)
				})}
			</div>
			...
			{/* Absolutely positioned segments, not flex weights: parallel tool calls
			    ...
			    consecutive slices that stretch the journey past its own duration. */}
			...
			<div className="flex items-baseline font-mono text-[11px] text-muted-foreground tabular-nums">
				{/* Keyed by position, not label: a sub-4s journey rounds two ticks to
...
function buildTicks(totalMs: number): Array<{ label: string }> {
	...
	return [
		{ label: "0s" },
		{ label: formatJourneyDuration(quarter) },
		{ label: formatJourneyDuration(quarter * 2) },
		{ label: formatJourneyDuration(quarter * 3) },
		{ label: formatJourneyDuration(totalMs) },
	]
}
...
export function JourneyPrivacyBanner({ contentInEvents }: { contentInEvents: boolean }) {
	return (
 ...
 */
export function JourneySystemPrompt({
	event,
	promptTokens,
}: {
	event: JourneyModel["systemPrompt"]
