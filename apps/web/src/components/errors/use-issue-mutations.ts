...
export function useIssueMutations() {
  ...
  const transitionMany = async (
    ...
    if (failed === 0) {
      toast.success(
        `Moved ${issueIds.length} to ${WORKFLOW_LABEL[toState]}`,
      )
    } else if (failed < issueIds.length) {
      toast.warning(
        `Moved ${issueIds.length - failed} of ${issueIds.length}; ${failed} failed`,
