...
export function IssueOccurrenceSparkline({
  data,
  className,
}: IssueOccurrenceSparklineProps) {
  ...
  const axisContext = React.useMemo(() => {
    ...
    return {
      ...
      bucketSeconds:
        diffMs > 0 && Number.isFinite(diffMs) ? diffMs / 1000 : undefined,
    }
  }, [sorted])
  ...
  return (
    <ChartContainer
      config={CHART_CONFIG}
      className={cn("aspect-auto h-20 w-full", className)}
    >
      <AreaChart
        data={sorted}
          ...
          content={
            <ChartTooltipContent
              labelFormatter={(value) =>
                formatBucketLabel(value, axisContext, "tooltip")
              }
              formatter={(value) => (
