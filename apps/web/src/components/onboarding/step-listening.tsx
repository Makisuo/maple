...
import { CircleCheckIcon, SunIcon, HouseIcon, PulseIcon, FileIcon, ClockIcon } from "@/components/icons"
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			...
			setFeedEntries((prev) => [
				...
				{
					time: formatTime(),
					message: prev.length % 2 === 0 ? "Polling endpoint..." : "No spans detected yet",
				},
			])
		}, 5000)
			...
			<AnimatePresence mode="wait">
				{detectedData ? (
						...
						{stats && (
							<div className="flex items-center gap-0 rounded-xl border divide-x">
								...
								<StatItem
									...
									className="text-severity-info"
								/>
								<StatItem value={`${Math.round(stats.avgLatency)}ms`} label="Avg Latency" />
								<StatItem
					...
					>
						...
						{/* Heading */}
						<div className="text-center space-y-2">
							<h2 className="text-2xl font-semibold tracking-tight">Listening for events...</h2>
							<p className="text-muted-foreground text-[15px] max-w-md">
								Run your instrumented app and trigger a request. We'll auto-detect when traces
								arrive.
							</p>
						</div>
						...
						<div className="flex items-center gap-3 text-sm text-muted-foreground">
							...
							<button type="button" onClick={onSkip} className="text-primary hover:underline">
								Skip for now
							</button>
						</div>
					</motion.div>
