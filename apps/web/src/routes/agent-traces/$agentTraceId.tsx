...
import {
	getAgentTraceSummaryResultAtom,
	getAgentTraceTimelineResultAtom,
} from "@/lib/services/atoms/warehouse-query-atoms"
import { useAgentTracingEnabled } from "@/hooks/use-agent-tracing-enabled"
import { AgentTraceDetail } from "@/components/agent-traces/detail/agent-trace-detail"
import { AgentTraceDetailSkeleton } from "@/components/agent-traces/detail/agent-trace-detail-skeleton"

// ---------------------------------------------------------------------------
// /agent-traces/$agentTraceId — one agent conversation, end to end.
//
// `t` is a partition-pruning hint (any warehouse timestamp inside the agent trace),
// set by the list row. It is deliberately optional: an agent trace id is the whole
// identity of this page, and a hand-built URL carrying only that id has to
// work. Absent, both queries fall back to a wide window — an agent trace clipped at
...
/** An agent trace is a conversation, not a session — 24h is a generous upper bound on one. */
const MAX_AGENT_TRACE_MS = 24 * 60 * 60 * 1000

interface AgentTraceWindow {
	readonly windowStart: string
	readonly windowEnd: string
}

function agentTraceWindow(hint: string | undefined): AgentTraceWindow | undefined {
	...
	return {
		...
		windowEnd: formatWarehouseDateTime(startMs + MAX_AGENT_TRACE_MS),
	}
}

export const Route = createFileRoute("/agent-traces/$agentTraceId")({
	...
	loader: ({ context, params, deps }) => {
		...
		const data = { agentTraceId: params.agentTraceId, ...agentTraceWindow(hint) }
		context.effectRegistry.mount(getAgentTraceSummaryResultAtom({ data }))
		context.effectRegistry.mount(getAgentTraceTimelineResultAtom({ data }))
	},
})
...
function AgentTraceDetailPage() {
	...
	return <AgentTraceDetailPageContent />
}

function AgentTraceDetailPageContent() {
	const { agentTraceId } = Route.useParams()
	...
	const data = useMemo(() => ({ agentTraceId, ...agentTraceWindow(hint) }), [agentTraceId, hint])

	const summaryAtom = getAgentTraceSummaryResultAtom({ data })
	const timelineAtom = getAgentTraceTimelineResultAtom({ data })
	...
	// A running agent trace is still being written. Both atom keys are stable (the id
	...
	const breadcrumbs = [
		...
		{ label: agentTraceId.slice(0, 12) },
	]

	return Result.builder(combined)
		.onInitial(() => (
			<Shell breadcrumbs={breadcrumbs} title="Loading agent trace…">
				<AgentTraceDetailSkeleton />
			</Shell>
		))
		.onError((error) => (
			<Shell breadcrumbs={breadcrumbs} title="Error">
				<QueryErrorState error={error} titleOverride="Failed to load agent trace" />
			</Shell>
		))
		.onSuccess(([summary, timeline]) => {
			// An agent trace with no spans in the window isn't an error — the id may be
			// real but older than the scan, which is a different sentence to "that
			// agent trace does not exist".
				...
				return (
					...
					>
						<div className="rounded-xl border border-dashed border-border p-12 text-center text-sm text-muted-foreground">
							Nothing for agent trace <span className="font-mono">{agentTraceId}</span>. It may
							have fallen outside the default 7-day window — deep-link with a{" "}
