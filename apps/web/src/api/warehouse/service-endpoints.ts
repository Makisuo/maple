...
import { DeploymentEnvironment, ServiceName, ServiceEndpointsRequest } from "@maple/domain/http"
import { MapleInternalAtomClient } from "@/lib/services/common/internal-atom-client"
import { WarehouseDateTimeString, decodeInput, runWarehouseQuery } from "@/api/warehouse/effect-utils"
import type { ServiceOperationSparklinePoint } from "@/api/warehouse/service-operations"

export interface ServiceEndpoint {
	/**
	 * Display span name ("GET /api/users"). This — not `route` — is the value the
	 * /traces `spanNames` filter matches, so keep it whole when linking out.
	 */
	spanName: string
	method: string
	route: string
