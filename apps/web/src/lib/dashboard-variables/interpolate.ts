...
/** Escape a value for embedding inside a single-quoted ClickHouse string literal. */
...
function sqlLiteral(variable: ResolvedVariable): string {
	if (variable.isAll) {
		...
		return variable.options.map((option) => `'${escapeSqlString(option)}'`).join(",")
	}
	return `'${escapeSqlString(variable.value)}'`
}
...
// the reference is removed rather than substituted.
