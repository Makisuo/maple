...
export const logs_lede = () =>
	`Structured logs streamed straight from OTLP. Severity, service, message, duration. Searchable in seconds, retained for 30 days on Startup — custom retention on Enterprise.`
...
export const features_errors_desc = () =>
	`Errors are automatically grouped by type across all your services. See trends over time, affected services, and drill into sample traces to find the root cause.`
...
export const usecases_ecommerce_desc = () =>
	`Your flash sale goes live and payment failures start climbing. You need to find the failing service, the exact error, and the upstream cause — fast.`
...
export const usecases_ecommerce_result = () =>
	`In this scenario: a Stripe timeout, correlated with retry-exhaustion logs — root cause in about 90 seconds.`
...
export const usecases_microservices_desc = () =>
	`No one deployed anything, but P95 latency just doubled. The service map shows which dependency is the bottleneck and how the cascade propagates.`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const usecases_microservices_result = () =>
...
export const usecases_ai_desc = () =>
	`An alert fires at 3 AM. Before your on-call is fully awake, an MCP agent connects to Maple, investigates the spike, and posts what it found to Slack.`
...
export const usecases_ai_result = () =>
	`In this scenario, the agent posts its diagnosis to #incidents before anyone wakes up.`
...
export const usecases_diagnosis_text = () =>
	`Stripe API timeout causing payment failures. Connection pool at capacity. Recommend circuit breaker activation.`
...
export const otel_future_desc = () =>
	`Backed by every major cloud provider. OpenTelemetry graduated from the CNCF in 2026 and is its second most active project after Kubernetes.`
...
export const otel_signals_desc = () =>
	`Traces, logs, and metrics from one SDK — no separate vendor agents to install and reconcile per signal.`
...
export const otel_community_desc = () =>
	`Thousands of contributors and auto-instrumentation for most major frameworks. The spec is driven by the community, not one vendor's roadmap.`
...
export const tech_columnar_desc = () =>
	`A columnar engine built for telemetry. Scans billions of rows and returns in well under a second — no pre-aggregation required.`
...
export const tech_connectors_desc = () =>
	`Pull data from Cloudflare Logpush and Prometheus scrape targets alongside your OpenTelemetry signals.`
...
export const ai_desc = () =>
	`Maple has a built-in MCP server and AI agent that query your traces, logs, errors, and metrics directly. Connect any MCP-compatible client and let agents investigate production issues and propose fixes.`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const ai_above_threshold = () =>
...
export const chat_desc = () =>
	`A built-in AI assistant that understands your traces, logs, and metrics. Ask questions in plain language and get answers with direct links to the relevant data.`
...
export const chat_response_1 = () =>
	`I found a latency spike starting at 14:12 UTC. The P95 jumped from 140ms to 890ms.`
...
export const chat_response_2 = () =>
	`Root cause: 3 slow database queries in the checkout flow averaging 720ms each. The connection pool hit its max limit (20/20).`
...
export const dashboard_desc = () =>
	`Drag-and-drop widgets, pick from pre-built presets, or let AI suggest the right charts for your services. Export and share as JSON.`
...
export const pricing_trial_reassure = (params) =>
	`Free for ${params.duration} days · Cancel anytime · Card required to start`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const pricing_calc_sub = () =>
	`Slide your monthly volume and compare your Maple cost against any vendor.`
...
export const faq_ingestion_a = () =>
	`Maple measures ingestion by the volume of data you send — logs, traces, and metrics are each measured in gigabytes. Each plan includes a monthly data allowance per signal type. Check the plans above for exact limits.`
...
export const faq_limits_a = () =>
	`We'll notify you as you approach your included volume. On the Startup plan, additional usage is billed at a flat $0.30 per GB — the same rate shown on this page.`
...
export const faq_retention_a = () =>
	`Retention varies by plan — the Startup plan includes 30-day retention and Enterprise plans offer custom retention periods. All plans include full query access to your retained data.`
...
export const faq_otel_a = () =>
	`Yes — Maple is built natively on OpenTelemetry. Any application instrumented with OTel SDKs or the OTel Collector can send data directly to Maple with zero code changes. No proprietary agents required.`
...
export const faq_trial_a = () =>
	`The Startup plan includes a 14-day free trial. A credit card is required, and you won't be charged until the trial ends. Enterprise plans start with a conversation instead.`
...
export const page_pricing_desc = () =>
	`Transparent usage-based pricing for logs, traces, and metrics. No per-seat fees, no hidden costs.`
...
export const page_pricing_subtitle = () =>
	`Usage-based pricing with no per-seat fees and no hidden costs. 14-day free trial on the Startup plan.`
...
export const page_otel_desc = () =>
	`Maple is built natively on OpenTelemetry. Ingest traces, logs, and metrics via OTLP with full spec compliance — no proprietary agents, no vendor lock-in.`
...
export const page_roadmap_desc = () =>
	`See what we're building, what's in progress, and what's shipped. Maple's development roadmap is transparent and updated regularly.`
...
export const page_roadmap_subtitle = () =>
	`A transparent view of Maple's development. See what's shipped, what's in progress, and where we're headed next.`
...
export const page_dt_subtitle = () =>
	`Visualize request flows with flamegraph, waterfall, and flow views. See exactly where time is spent across your distributed system.`
...
export const page_dt_waterfall_view_desc = () =>
	`See the full timeline of a request with nested spans, timing bars, and service colors.`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const page_dt_flamegraph_desc = () =>
	`Identify hot paths and time-consuming operations with interactive flamegraph visualization.`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const page_dt_flow_desc = () =>
	`Understand service-to-service communication with an interactive dependency flow diagram.`
...
export const page_dt_attributes_desc = () =>
	`Inspect HTTP headers, database queries, error messages, and custom attributes on any span.`
...
export const page_dt_correlation_desc = () =>
	`Jump from any span directly to its correlated logs. Every signal connected by trace ID.`
...
export const page_dt_filtering_desc = () =>
	`Filter traces by service, duration, status, HTTP method, and custom attributes.`
...
export const page_k8s_desc = () =>
	`Monitor Kubernetes clusters with OpenTelemetry. A Helm chart collects kubelet, host, and kube-state metrics; spans arrive enriched with pod, node, and namespace.`
 ...
 */
/* @__NO_SIDE_EFFECTS__ */
export const page_k8s_workloads_subtitle = () =>
...
export const page_dd_desc = () =>
	`Compare Maple and Datadog for observability. Maple offers transparent usage-based pricing, native OpenTelemetry support, and AI-powered diagnostics — open source.`
...
export const page_dd_subtitle = () =>
	`Datadog is powerful but comes with per-host pricing and proprietary agents. Maple covers distributed tracing, logs, and metrics with a flat per-GB rate, native OpenTelemetry ingest, and source you can read and self-host.`
...
export const page_dd_otel_desc = () =>
	`Built on OpenTelemetry from the start. No proprietary agents to install or maintain — the OTel instrumentation you already have points straight at Maple.`
...
export const page_dd_oss_desc = () =>
	`The source is on GitHub under FSL-1.1, converting to Apache 2.0 over time. Inspect every line, contribute features, and see exactly how your data is handled.`
...
export const page_ecommerce_desc = () =>
	`Debug checkout failures, payment timeouts, and inventory issues in real-time with distributed tracing and log correlation.`
...
export const page_ecommerce_subtitle = () =>
	`When your flash sale hits and payments start failing, Maple shows you exactly what's happening across every service.`
...
export const page_ecommerce_step1_desc = () =>
	`Your monitoring detects an error rate spike on the payment service. The alert includes the threshold breach and affected service, giving you immediate context.`
...
export const page_ecommerce_step2_desc = () =>
	`Click into a failing trace to see the full request path. The payment span is immediately visible in red, showing a 5000ms timeout on the Stripe API call.`
...
export const page_ecommerce_step3_desc = () =>
	`Jump to correlated logs for the failing span. The Stripe API timeout and retry exhaustion are immediately visible, confirming the root cause without switching tools.`
...
export const page_ecommerce_cap_tracing_desc = () =>
	`Follow checkout requests from cart to payment to confirmation. See every service hop with timing.`
...
export const page_ecommerce_cap_logs_desc = () =>
	`Every failed payment links to its retry logs and timeout errors. One click from trace to logs.`
...
export const page_ecommerce_cap_errors_desc = () =>
	`Group payment errors by type, see trends, fix systematically. No more searching through log files.`
...
export const page_nextjs_desc = () =>
	`Add distributed tracing, logging, and metrics to your Next.js app in minutes. Built on OpenTelemetry with zero vendor lock-in.`
...
export const feat_tracing_seo_desc = () =>
	`Open one trace and read where the time went — waterfall, flamegraph and service flow over the same spans, with every attribute the SDK sent.`
...
export const feat_sessions_seo_desc = () =>
	`Watch the session the user actually had, then open the trace behind any moment in it. Replay, console, network and spans share one session id.`
...
export const feat_logs_seo_desc = () =>
	`Search structured logs across every service, filter by severity and resource attribute, and open the trace that produced any line.`
...
export const feat_metrics_seo_desc = () =>
	`Browse every metric your services emit, then build dashboards on the same warehouse the trace list reads from — so a chart and a trace can never disagree.`
...
export const feat_catalog_seo_desc = () =>
	`Every service with its latency percentiles, error rate and throughput, and a live map of who calls whom — both built from spans, not from a config file.`
...
export const feat_errors_seo_desc = () =>
	`Exceptions grouped into issues by type and normalised message, each one linked to the span that threw it and the release that introduced it.`
...
export const feat_alerts_seo_desc = () =>
	`Threshold alerts on traces, logs and metrics — previewed against your own history before you save, evaluated every minute, and delivered to Slack, PagerDuty, Discord, email or any webhook.`
...
export const feat_mcp_seo_desc = () =>
	`Maple ships an MCP server, so Claude Code and any other MCP client can read your traces, logs and metrics and work an incident with the same tools you do.`
...
export const uc_api_seo_desc = () =>
	`An SLO breach, traced to the release that caused it. Follow checkout p99 from the alert through the dashboard to the commit that took it from 120 ms to 380 ms.`
...
export const uc_ecom_seo_desc = () =>
	`Checkout failures during a flash sale, traced from the alert to the Stripe timeout and the retry-exhaustion logs behind it — one order id the whole way.`
...
export const uc_micro_seo_desc = () =>
	`Latency doubled with no deploy in sight. Follow one release across the service map, a trace and its logs to the connection pool that ran dry two hops away.`
