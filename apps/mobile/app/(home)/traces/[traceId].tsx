...
export default function TraceDetailScreen() {
	...
	// Derive header info from root span
	const headerInfo = useMemo(() => {
		...
		const httpStatusCode = root.spanAttributes["http.status_code"] || root.spanAttributes["http.response.status_code"]
		...
		<View className="flex-1 bg-background">
			...
			<View className="px-5 pt-16 pb-2">
				...
				<Pressable
					...
					hitSlop={8}
				>
					<Text className="text-sm text-primary font-mono">{"\u2039"} Traces</Text>
				</Pressable>

				{state.status === "success" && headerInfo ? (
					<>
						{/* Method + Route */}
						<View className="flex-row items-center mb-2">
							{headerInfo.method && (
								<View
									...
									style={{ backgroundColor: HTTP_METHOD_COLORS[headerInfo.method] ?? "#5A5248" }}
								>
							...
							<Text
								className="text-lg font-bold text-foreground font-mono flex-1"
								numberOfLines={1}
							>
								{headerInfo.route}
							</Text>
						...
						<View className="flex-row flex-wrap gap-2 mb-3">
							...
							{headerInfo.statusCode != null && (
								<View
									...
									style={{
										backgroundColor: getStatusBgColor(headerInfo.statusCode, headerInfo.hasError),
									}}
								>
									<Text
			...
			<View className="flex-row px-5 border-b border-border">
				<TabButton label="Waterfall" active={activeTab === "waterfall"} onPress={() => setActiveTab("waterfall")} />
				<TabButton label="Details" active={activeTab === "details"} onPress={() => setActiveTab("details")} />
				<TabButton
					label={state.status === "success" ? `Logs` : "Logs"}
			...
			{state.status === "loading" ? (
				...
				<View className="flex-1 items-center justify-center px-5">
					...
					<Text className="text-sm text-primary font-mono mt-3" onPress={refresh}>
						Tap to retry
					</Text>
				</View>
			) : activeTab === "waterfall" ? (
