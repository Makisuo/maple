...
describe("jsonSchemaToValibot", () => {
	...
	it("converts enums, including boolean members picklist cannot hold", () => {
		expect(parse({ type: "object", properties: { m: { enum: ["p50", "p95"] } } }, { m: "p95" }).success).toBe(
			true,
		)
		expect(parse({ type: "object", properties: { m: { enum: ["p50", "p95"] } } }, { m: "p99" }).success).toBe(
			false,
		)
		expect(parse({ type: "object", properties: { on: { enum: [true, false] } } }, { on: true }).success).toBe(
			true,
		)
	})
	...
	it("resolves $ref into $defs", () => {
		const schema = {
			...
			$defs: { Filter: { type: "object", properties: { field: { type: "string" } }, required: ["field"] } },
		}
	...
	it("handles arrays, unions and nullable types", () => {
		expect(parse({ type: "object", properties: { ids: { type: "array", items: { type: "string" } } } }, {
			ids: ["a", "b"],
		}).success).toBe(true)
		expect(
			parse({ type: "object", properties: { v: { anyOf: [{ type: "string" }, { type: "number" }] } } }, {
				v: 3,
			}).success,
		).toBe(true)
		expect(parse({ type: "object", properties: { v: { type: ["string", "null"] } } }, { v: null }).success).toBe(
			true,
		)
	})

	it("degrades an unrecognised property to unknown rather than dropping the tool", () => {
		expect(parse({ type: "object", properties: { weird: {} } }, { weird: { any: "thing" } }).success).toBe(true)
	})
