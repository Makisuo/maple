  ...
  Command.withHandler(
    Effect.fnUntraced(function* (f) {
      ...
      if (Option.isSome(f.around)) {
        ...
        Option.isSome(f.currentStart) && Option.isSome(f.currentEnd) &&
        Option.isSome(f.previousStart) && Option.isSome(f.previousEnd)
      ) {
        ...
        yield* Console.error("Provide --around or all four --current-start/end --previous-start/end flags")
        return
      }
