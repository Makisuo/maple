...
export const timeseries = Command.make("timeseries", {
  ...
  metric: Flag.string("metric").pipe(
    Flag.withDescription("Metric: count, avg_duration, p50_duration, p95_duration, p99_duration, error_rate, apdex"),
    Flag.withDefault("count"),
  ),
  service: flags.service,
  span: Flag.optional(
    Flag.string("span").pipe(
      Flag.withDescription("Filter by span name"),
    ),
  ),
  ...
  bucket: Flag.optional(
    Flag.integer("bucket").pipe(
      Flag.withDescription("Bucket size in seconds"),
    ),
  ),
  json: flags.json,
}).pipe(
