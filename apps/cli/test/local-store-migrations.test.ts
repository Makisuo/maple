...
describe("current local schema identity", () => {
	it("matches the generated v15 revision and keeps the issue-297 identity frozen", () => {
		expect(SCHEMA_FINGERPRINT).toBe("9dcc61f3d2522826")
		expect(SCHEMA_DIGEST).toBe("9dcc61f3d2522826070ac2063c777d34ec4b1f4492d4ac6663729c21c29a4495")
		...
		expect(CURRENT_LOCAL_SCHEMA.version).toBe(15)
		expect(CURRENT_LOCAL_SCHEMA).toEqual(LOCAL_SCHEMA_V15)
		...
		// `identity_links`; v15 adds `audit_log`. Asserted as an exact set difference rather than a
		...
		expect([...currentNames].filter((name) => !v5Names.has(name))).toEqual([
			"ai_trace_index",
			"ai_trace_index_mv",
			// v15: the org audit trail, empty in local mode (no authenticated actors).
			"audit_log",
			"identity_links",
			"identity_links_mv",
			"product_events",
		...
		// empty and filled forward; v15 is exactly the audit trail, created empty.
