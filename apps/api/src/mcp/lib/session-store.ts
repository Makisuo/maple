...
export const runWithSessionBindings = <T>(bindings: RequestBindings, fn: () => T): T =>
	requestStore.run(bindings, fn)
...
export class SessionStore extends Map<string, SessionPayload> {
	async preload(sessionId: string): Promise<void> {
		...
		if (!store?.kv) return
		try {
			const value = (await store.kv.get(sessionId, "json")) as SessionPayload | null
