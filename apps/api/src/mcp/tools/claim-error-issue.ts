...
export function registerClaimErrorIssueTool(server: McpToolRegistrar) {
  server.tool(
    ...
    Effect.fn("McpTool.claimErrorIssue")(function* ({
      issue_id,
      ...
      const issue = yield* errors
        .claimIssue(tenant.orgId, actorId, decodedIssueId.value, leaseMs)
        .pipe(
          Effect.mapError(
            (error) =>
              new McpQueryError({
      ...
      if (!issue.leaseHolder || !issue.leaseExpiresAt || !issue.claimedAt) {
        return validationError(
          "Claim succeeded but lease fields were not populated on the issue.",
        )
      }
