	...
	server.tool(
		...
		Schema.Struct({
			...
			visualization: requiredStringParam(
				'MUST be exactly one of: "chart", "stat", "gauge", "table", "list", "pie", "histogram", "heatmap", "funnel", "markdown". This is the widget KIND, not a title — set the title via `display_json.title`. For line/area/bar charts use `"chart"` and set `display_type` to `"line"`/`"area"`/`"bar"`.',
			),
