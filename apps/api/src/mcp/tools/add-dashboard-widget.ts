...
import { isMcpVisualization, MCP_VISUALIZATIONS, RawSqlDisplayType } from "@maple/domain/http"
...
// Rendered into the tool and parameter descriptions below. The list used to be
// retyped by hand in both places even though the error message already derived
// it, so a newly `mcpExposed` widget type was advertised in one voice and
// rejected in another.
const VISUALIZATION_LIST_MD = KNOWN_VISUALIZATIONS.map((viz) => `\`${viz}\``).join(", ")
...
export function registerAddDashboardWidgetTool(server: McpToolRegistrar) {
	server.tool(
		...
		Schema.Struct({
			...
			visualization: requiredStringParam(
				"MUST be exactly one of: " +
					...
					'. This is the widget KIND, not a title — set the title via `display_json.title`. For line/area/bar charts use `"chart"` and set `display_type` to `"line"`/`"area"`/`"bar"`.',
			),
		...
		Effect.fn("McpTool.addDashboardWidget")(function* ({
			...
			if (!isMcpVisualization(visualization)) {
