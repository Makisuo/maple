...
export function registerCommentOnErrorIssueTool(server: McpToolRegistrar) {
  server.tool(
    ...
    Effect.fn("McpTool.commentOnErrorIssue")(function* ({
      issue_id,
      body,
      kind,
      visibility,
      ...
      const kindTyped: "comment" | "agent_note" =
        kind === "agent_note" ? "agent_note" : "comment"
      ...
      const event = yield* errors
        .commentOnIssue(
          tenant.orgId,
          actorId,
          decodedIssueId.value,
          body,
          { kind: kindTyped, visibility: visibilityTyped },
        )
        .pipe(
          Effect.mapError(
            (error) =>
