...
export function registerProposeFixTool(server: McpToolRegistrar) {
  server.tool(
    ...
    Schema.Struct({
      ...
      patch_summary: requiredStringParam(
        "Short description of the proposed fix (1..4000 chars)",
      ),
      pr_url: optionalStringParam("Link to PR, diff, or patch"),
      artifacts_json: optionalStringParam(
        "JSON array of artifact URLs (logs, traces, analysis docs)",
      ),
    }),
    Effect.fn("McpTool.proposeFix")(function* ({
      issue_id,
      patch_summary,
      pr_url,
      artifacts_json,
