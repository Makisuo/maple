...
const defaultAppLayer = (env: Record<string, unknown>) =>
	DatabaseD1Live.pipe(Layer.provide(Layer.succeed(WorkerEnvironment, env)))

const runStep = <A, E>(appLayer: Layer.Layer<Database>, effect: Effect.Effect<A, E, Database>): Promise<A> =>
...
const readSyncRun = Effect.fn("TinybirdSyncWorkflow.readSyncRun")(function* (orgId: OrgId) {
	...
	const rows = yield* database.execute((db) =>
		db.select().from(orgTinybirdSyncRuns).where(eq(orgTinybirdSyncRuns.orgId, orgId)).limit(1),
	)
	const row = rows[0]
	if (!row) {
...
const updateSyncRun = Effect.fn("TinybirdSyncWorkflow.updateSyncRun")(function* (
	orgId: OrgId,
	patch: SyncRunPatch,
) {
	yield* Effect.annotateCurrentSpan("orgId", orgId)
...
const promoteActiveConfig = Effect.fn("TinybirdSyncWorkflow.promoteActiveConfig")(function* (
	orgId: OrgId,
	row: SyncRunSnapshot,
...
const resolveToken = Effect.fn("TinybirdSyncWorkflow.resolveToken")(function* (
...
export const runTinybirdSyncWorkflow = async (
	...
	const publishCollectorConfigStep = async () => {
		...
		await step.do("publish-self-managed-collector-config", DEFAULT_STEP_CONFIG, async () => {
			try {
	...
	try {
		...
		if (started.result === "no_changes") {
			...
			await step.do("promote-no-changes", DEFAULT_STEP_CONFIG, async () => {
				await runStep(appLayer, promoteActiveConfig(orgId, snapshot, started.projectRevision, null))
				await runStep(
					appLayer,
					updateSyncRun(orgId, {
						runStatus: "succeeded",
