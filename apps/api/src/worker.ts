...
export default class MapleApiWorker extends WorkerEntrypoint<Record<string, unknown>> {
	...
	// Dispatch on the firing schedule so the billing reconcile cron is routed
	// separately from the 12h VCS sync (see handleScheduled).
	override scheduled(event: ScheduledController): Promise<void> {
		return handleScheduled(event.cron, this.env, this.ctx)
	}
