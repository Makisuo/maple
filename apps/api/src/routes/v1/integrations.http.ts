	...
	Effect.gen(function* () {
		...
		return (
			handlers
				...
				.handle("cloudflareDisconnectAccount", ({ params }) =>
					Effect.gen(function* () {
						const tenant = yield* CurrentTenant.Context
						yield* requireAdmin(tenant.roles)
						const result = yield* cloudflare.disconnect(tenant.orgId, params.accountId)
						return new CloudflareDisconnectResponse(result)
					}),
				)
				.handle("githubStatus", () =>
	...
	Effect.gen(function* () {
		...
		const handleCloudflare = Effect.fn("integrations.cloudflareOAuthCallback")(function* (
			...
			return yield* cloudflare.completeConnect(code, state).pipe(
				...
				Effect.tap((result) =>
					cloudflareAnalytics.resetOrgState(result.orgId, result.accountId).pipe(
