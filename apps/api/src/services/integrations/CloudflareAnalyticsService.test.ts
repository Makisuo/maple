...
/** Insert a connected Cloudflare account with a non-expiring encrypted access token. */
const seedConnection = (
	scope: string = ANALYTICS_SCOPE,
	account: { id: string; name: string; connectedAt?: number } = { id: ACCOUNT_ID, name: "Test Account" },
) =>
	Effect.gen(function* () {
		...
		yield* database.execute((db) =>
			db.insert(oauthConnections).values({
				...
				expiresAt: null,
				createdAt: new Date(account.connectedAt ?? T0 - 60 * MIN),
				updatedAt: new Date(account.connectedAt ?? T0 - 60 * MIN),
			}),
		)
	})
...
describe("CloudflareAnalyticsService", () => {
	...
	it.effect("pollOrg polls every connected account, each with its own state rows and zones", () => {
		const testDb = createTestDb(trackedDbs)
		const captured: CapturedIngest[] = []
		...
		return Effect.gen(function* () {
			...
			yield* seedConnection(ANALYTICS_SCOPE, {
				id: SECOND_ACCOUNT,
				name: "Second Account",
				// Later createdAt keeps account ordering (oldest first) deterministic.
				connectedAt: T0 - 30 * MIN,
			})
			const service = yield* CloudflareAnalyticsService
			const summary = yield* service.pollOrg(ORG)
			assert.isNull(summary.skipped)
