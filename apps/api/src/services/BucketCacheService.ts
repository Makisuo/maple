...
export interface BucketCacheServiceShape {
  ...
  readonly getOrComputeBuckets: <E, R>(
    ...
    computeRange: (
      range: TimeRange,
    ) => Effect.Effect<ReadonlyArray<TimeseriesPoint>, E, R>,
...
const fluxSecondsConfig = Config.number("QE_BUCKET_CACHE_FLUX_SECONDS").pipe(
  Config.withDefault(60),
)

export class BucketCacheService extends Context.Service<
  BucketCacheService,
  BucketCacheServiceShape
>()("BucketCacheService", {
      ...
      Effect.gen(function* () {
        ...
        const existingBuckets = Option.match(cached, {
          ...
          onSome: (data) =>
            ...
            data.bucketSeconds === request.bucketSeconds
              ? data.buckets
              : EMPTY_BUCKETS,
        })
        ...
        const composite = `${fingerprint}|${fingerprintRange(
          missing.map((m) => m.range),
        )}`
