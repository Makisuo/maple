...
const TRANSITIONS: Record<WorkflowState, ReadonlySet<WorkflowState>> = {
  triage: new Set<WorkflowState>([
    "todo",
    "in_progress",
    "cancelled",
    "wontfix",
  ]),
  todo: new Set<WorkflowState>([
    "triage",
    "in_progress",
    "cancelled",
    "wontfix",
  ]),
  in_progress: new Set<WorkflowState>([
    "triage",
    "todo",
    "in_review",
    "cancelled",
  ...
  in_review: new Set<WorkflowState>([
    "triage",
    "in_progress",
    "done",
    "cancelled",
...
export interface ErrorsServiceShape {
  ...
  readonly listAgents: (
    orgId: OrgId,
  ) => Effect.Effect<ActorsListResponse, ErrorPersistenceError>
...
export class ErrorsService extends Context.Service<ErrorsService, ErrorsServiceShape>()(
  "ErrorsService",
  {
    make: Effect.gen(function* () {
      ...
      const isoFromEpoch = (ms: number) =>
        decodeIsoDateTimeStringSync(new Date(ms).toISOString())
      ...
      const rowToActor = (row: ActorRow): ActorDocument =>
        new ActorDocument({
          ...
          lastActiveAt:
            row.lastActiveAt == null ? null : isoFromEpoch(row.lastActiveAt),
        })
      ...
      const lookupActor: ErrorsServiceShape["lookupActor"] = Effect.fn(
        "ErrorsService.lookupActor",
      ...
      const ensureUserActor: ErrorsServiceShape["ensureUserActor"] = Effect.fn(
        ...
        yield* dbExecute((db) =>
          db.insert(actors).values(insert).onConflictDoNothing(),
        )
      ...
      const registerAgent: ErrorsServiceShape["registerAgent"] = Effect.fn(
        "ErrorsService.registerAgent",
      ...
      const listAgents: ErrorsServiceShape["listAgents"] = Effect.fn(
        "ErrorsService.listAgents",
      )(function* (orgId) {
        const rows = yield* dbExecute((db) =>
          db
            .select()
      ...
      const collectActorDocs = (
        orgId: OrgId,
        actorIds: ReadonlyArray<ActorId | null>,
      ) => {
        const filtered = Array.from(
          new Set(actorIds.filter((v): v is ActorId => v != null)),
        )
        ...
        new ErrorIssueDocument({
          ...
          assignedActor:
            row.assignedActorId == null
              ? null
              : actorMap.get(row.assignedActorId) ?? null,
          leaseHolder:
            row.leaseHolderActorId == null
              ? null
              : actorMap.get(row.leaseHolderActorId) ?? null,
          leaseExpiresAt:
            row.leaseExpiresAt == null
              ? null
              : isoFromEpoch(row.leaseExpiresAt),
          ...
          resolvedAt:
            row.resolvedAt == null ? null : isoFromEpoch(row.resolvedAt),
          snoozeUntil:
            row.snoozeUntil == null ? null : isoFromEpoch(row.snoozeUntil),
          archivedAt:
            row.archivedAt == null ? null : isoFromEpoch(row.archivedAt),
          hasOpenIncident,
        })
      ...
      const listIssues: ErrorsServiceShape["listIssues"] = Effect.fn(
        ...
        if (opts.workflowState)
          conditions.push(eq(errorIssues.workflowState, opts.workflowState))
        ...
        if (!opts.includeArchived)
          conditions.push(isNull(errorIssues.archivedAt))
        if (opts.endTime) {
          const endMs = Date.parse(opts.endTime)
          if (Number.isFinite(endMs))
            conditions.push(lt(errorIssues.firstSeenAt, endMs))
        }
        if (opts.startTime) {
          const startMs = Date.parse(opts.startTime)
          if (Number.isFinite(startMs))
            conditions.push(gt(errorIssues.lastSeenAt, startMs))
        }
        ...
        const actorMap = yield* collectActorDocs(
          orgId,
          rows.flatMap((r) => [
            r.assignedActorId ?? null,
            r.leaseHolderActorId ?? null,
          ]),
        )

        const issuesResult = rows.map((r) =>
          rowToIssue(r, openSet.has(r.id), actorMap),
        )
      ...
      const applyTransition = Effect.fn("ErrorsService.applyTransition")(
        function* (
          orgId: OrgId,
          actorId: ActorId | null,
          row: ErrorIssueRow,
          ...
          yield* dbExecute((db) =>
            db
              ...
              .set(update)
              .where(
                and(eq(errorIssues.orgId, orgId), eq(errorIssues.id, row.id)),
              ),
          )
              ...
              db
                ...
                .where(
                  and(
                    ...
                    eq(errorIssueStates.issueId, row.id),
                  ),
                ),
            )
          }
      ...
      const claimIssue: ErrorsServiceShape["claimIssue"] = Effect.fn(
        "ErrorsService.claimIssue",
      ...
      const heartbeatIssue: ErrorsServiceShape["heartbeatIssue"] = Effect.fn(
        ...
        const leaseMs = Math.max(
          ...
          previous - (current.claimedAt ?? previous),
        )
        const leaseExpiresAt = timestamp + leaseMs
        yield* dbExecute((db) =>
          db
      ...
      const releaseIssue: ErrorsServiceShape["releaseIssue"] = Effect.fn(
        "ErrorsService.releaseIssue",
      )(function* (orgId, actorId, issueId, opts) {
        const timestamp = now()
        const current = yield* requireIssue(orgId, issueId)
        if (
          current.leaseHolderActorId !== null &&
        ...
        yield* dbExecute((db) =>
          db
            ...
            .where(
              and(eq(errorIssues.orgId, orgId), eq(errorIssues.id, issueId)),
            ),
        )
      ...
      const assignIssue: ErrorsServiceShape["assignIssue"] = Effect.fn(
      ...
      const commentOnIssue: ErrorsServiceShape["commentOnIssue"] = Effect.fn(
        ...
        const timestamp = now()
        yield* requireIssue(orgId, issueId)
        const type: ErrorIssueEventType =
          opts?.kind === "agent_note" ? "agent_note" : "comment"
      ...
      const proposeFix: ErrorsServiceShape["proposeFix"] = Effect.fn(
        "ErrorsService.proposeFix",
      ...
      const upsertNotificationPolicy: ErrorsServiceShape["upsertNotificationPolicy"] =
        Effect.fn("ErrorsService.upsertNotificationPolicy")(function* (
          ...
          const merged: ErrorNotificationPolicyRow = {
            ...
            notifyOnTransitionDone: toFlag(
              request.notifyOnTransitionDone,
              base.notifyOnTransitionDone,
            ),
      ...
      const notifyIncidentOpened = (
        ...
        if (params.reason === "first_seen" && policy.notifyOnFirstSeen !== 1)
          return Effect.void
        if (params.reason === "regression" && policy.notifyOnRegression !== 1)
          return Effect.void
      ...
      const maybeNotifyTransition = (
        ...
        Effect.gen(function* () {
          ...
          const destinationIds = parsePolicyDestinations(
            policyRow.destinationIdsJson,
          )
      ...
      const expireLeasesForOrg = Effect.fn("ErrorsService.expireLeases")(
      ...
      const processOrg = Effect.fn("ErrorsService.processOrg")(function* (
        ...
        const issuesTouched = fingerprintResults.reduce(
          (s, r) => s + r.touched,
          0,
        )
        const incidentsOpened = fingerprintResults.reduce(
          (s, r) => s + r.opened,
          0,
        )
