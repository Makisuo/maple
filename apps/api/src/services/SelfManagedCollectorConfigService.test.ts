...
import { cleanupTempDirs, createTempDbUrl as makeTempDb, executeSql } from "./test-sqlite"
...
describe("renderCollectorConfig (pure)", () => {
	...
	it("rejects orgIds that would be unsafe as YAML keys", () => {
		...
		expect(() =>
			renderCollectorConfig([{ orgId: "org with space", host: "https://h", token: "t" }]),
		).toThrow(/Unsafe orgId/)
		expect(() => renderCollectorConfig([{ orgId: "org/slash", host: "https://h", token: "t" }])).toThrow(
			/Unsafe orgId/,
		)
	})
})
...
const insertActiveOrg = async (dbPath: string, opts: { orgId: string; host: string; token: string }) => {
...
const insertInactiveOrg = async (dbPath: string, opts: { orgId: string; host: string; token: string }) => {
	const encrypted = await Effect.runPromise(
		encryptAes256Gcm(opts.token, encryptionKey, (message) => new Error(message)),
	)
	const now = Date.now()
...
describe("SelfManagedCollectorConfigService", () => {
	...
	it("generateConfig renders a fallback-only YAML when no active orgs exist", async () => {
		const config = await Effect.runPromise(
			SelfManagedCollectorConfigService.generateConfig().pipe(Effect.provide(makeLayer(dbUrl))),
		)
	...
	it("publishConfig PUTs the generated YAML to the reload endpoint with auth", async () => {
		...
		const result = await Effect.runPromise(
			SelfManagedCollectorConfigService.publishConfig().pipe(Effect.provide(makeLayer(dbUrl))),
		)
	...
	it("publishConfig fails with PublishError when reload endpoint returns non-2xx", async () => {
		globalThis.fetch = vi.fn(
			async () => new Response("server off", { status: 503 }),
		) as unknown as typeof fetch
...
describe("SelfManagedCollectorConfigService (no reload URL)", () => {
	...
	it("publishConfig skips the HTTP call and reports published=false", async () => {
		...
		const result = await Effect.runPromise(
			SelfManagedCollectorConfigService.publishConfig().pipe(Effect.provide(makeLayerNoReload(dbUrl))),
		)
