  ...
  new OrgOpenrouterSettingsPersistenceError({
    message:
      error instanceof Error
        ? error.message
        : "Org OpenRouter settings persistence failed",
  })

const toEncryptionError = (message: string) =>
  new OrgOpenrouterSettingsEncryptionError({ message })

const parseEncryptionKey = (
  raw: string,
): Effect.Effect<Buffer, OrgOpenrouterSettingsEncryptionError> =>
...
const normalizeApiKey = (
  raw: string,
): Effect.Effect<string, OrgOpenrouterSettingsValidationError> =>
...
const toLast4 = (apiKey: string): string =>
  apiKey.slice(Math.max(0, apiKey.length - 4))
...
const toResponse = (row: ActiveRow | null | undefined): OrgOpenrouterSettingsResponse =>
  new OrgOpenrouterSettingsResponse({
    ...
    updatedAt:
      row == null
        ? null
        : decodeIsoDateTimeStringSync(new Date(row.updatedAt).toISOString()),
  })
...
>()("OrgOpenRouterSettingsService", {
  make: Effect.gen(function* () {
    ...
    const selectActiveRow = Effect.fn("OrgOpenRouterSettingsService.selectActiveRow")(
    ...
    const resolveApiKey = Effect.fn("OrgOpenRouterSettingsService.resolveApiKey")(
      function* (orgId: OrgId) {
        const row = yield* selectActiveRow(orgId)
  ...
  static readonly resolveApiKey = (orgId: OrgId) =>
    this.use((service) => service.resolveApiKey(orgId))
}
