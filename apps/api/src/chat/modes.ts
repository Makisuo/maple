...
//
// Moved into apps/api from apps/chat-flue when the chat shell came off Flue. The mode itself now
// lives in the shared wire contract (`chatModeFromSessionId` in `@maple/domain/chat-session`), so
// the client, the Durable Object and this prompt assembly all derive it the same way; only the
// prompt text and the context blocks are here.
...
/**
 * Assemble the full system prompt for a turn: the base prompt for the mode plus any attached
 * context blocks.
 */
