//! Rust↔SQL equivalence fixtures: adversarial spans, driven through the **real**
//! row writer.
//!
//! # The claim
//!
//! For any span, the SQL compiled from `registry.json`
//! (`packages/domain/src/ai-registry/compile-sql.ts`) evaluated over the row ingest
//! *actually wrote* produces the same `AiVendor` and `AiSessionKeyState` as the Rust
//! classifier that wrote it, and `AiSessionKeyHash` is byte-identical to
//! `cityHash64(value)` computed inside ClickHouse.
//!
//! This module owns the Rust half: it generates a deterministic corpus of adversarial
//! spans (write-side plan §6's fuzz surface), runs each one through [`super::encode_traces`]
//! — the same function the ingest request path calls, with the classification flag on
//! — and serialises **the exact NDJSON row the writer produced**
//! plus the Rust verdict as a JSONL artifact. The TypeScript half
//! (`packages/domain/src/ai-registry/equivalence.clickhouse.e2e.test.ts`) replays that
//! artifact into a real ClickHouse through the same generated INSERT shape ingest uses
//! and compares.
//!
//! Because the Rust verdict is already *on the row* (`ai_vendor`,
//! `ai_session_key_state`), the differential needs no join key: it is a single
//! `WHERE computed != stored` over the inserted rows. The `rust` block on each fixture
//! record is a redundant cross-check, and the generator asserts the two agree before
//! writing a line.
//!
//! # What this proves, and what it does not
//!
//! Rust↔SQL only. The trace-capture reference evaluator (`scripts/verify-seed.ts`) is a
//! third implementation and is **not** in this loop: it renders kvlist values in
//! insertion order and `JSON.stringify`s bytes, where `any_value_string` sorts kvlist
//! keys (the row Map is a `serde_json::Map`) and hex-encodes bytes. That divergence
//! cannot affect this suite — SQL reads the string the row writer already produced —
//! but it means "the fixture agrees with the seed verifier" is not a claim made here.
//!
//! # Determinism
//!
...
//!   cargo test -p maple-ingest --lib write_equivalence_fixture -- --ignored --nocapture
//! ```
//!
//! and, for the full capture corpus (local only, never checked in):
//!
//! ```sh
...
const ORG_ID: &str = "org_equivalence";
...
impl Group {
    fn new(scope_name: &str, cases: Vec<Case>) -> Self {
        Self {
            resource: vec![s("service.name", "equivalence-fixture")],
    ...
    vec![
        ...
        one(
            ...
            case(
                ...
                "a vendor attr hit outranks the unknown tier it overlaps",
                "ai.generateText",
                vec![
    ...
    vec![
        ...
        one(
            ...
            case(
                "resolution/unknown_tier",
                "unknown/ai_prefix",
                "key_prefix(ai.) with no vercel-specific evidence — still vercel's attr family",
                "call",
    ...
    out.push(Group::new(
        ...
        vec![
            case(
                ...
                "no AGENT kind, no agno.workflow. key",
                "Model.invoke",
...
/// Duplicate keys. Registry-referenced keys are first-occurrence-wins in *both* the
/// classifier and the row Map; every other key keeps the last occurrence.
    ...
    for (position, group) in groups.iter().enumerate() {
        ...
        for (case, row_text) in group.cases.iter().zip(rows) {
            ...
            let record = json!({
                ...
                // The exact NDJSON line the writer produced, verbatim. Kept as a string
                // so the ClickHouse leg can replay the bytes rather than a re-serialized
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../packages/domain/src/ai-registry/__fixtures__/equivalence-spans.jsonl")
}
...
#[test]
#[ignore = "regeneration: set EQUIVALENCE_FIXTURE_OUT"]
fn write_equivalence_fixture() {
    let Some(out) = std::env::var_os("EQUIVALENCE_FIXTURE_OUT") else {
        panic!("set EQUIVALENCE_FIXTURE_OUT to the artifact path");
    };
...
/// Regeneration is byte-stable. Skips when the artifact is absent (a fresh checkout of
/// apps/ingest alone still builds), fails loudly when it is present and stale.
#[test]
fn fixture_is_reproducible() {
    let path = fixture_path();
    let Ok(checked_in) = std::fs::read_to_string(&path) else {
    ...
    if regenerated != checked_in {
        ...
        panic!(
            "{} is stale ({} lines checked in, {} regenerated, first differing line {:?}). \
             Regenerate with EQUIVALENCE_FIXTURE_OUT=<path> cargo test --lib write_equivalence_fixture -- --ignored",
            path.display(),
            expected.len(),
            actual.len(),
            first_difference
...
// ---------------------------------------------------------------------------
// full-corpus replay (local only)
// ---------------------------------------------------------------------------

/// Emits row+verdict JSONL for every span in the trace-capture corpus, through the same
/// real row writer. Local-only: the corpus is a sibling repo, not a vendored fixture.
///
/// ```sh
/// TRACE_CAPTURE_DIR=~/Documents/repos/trace-capture \
/// CORPUS_EQUIVALENCE_OUT=/tmp/corpus-equivalence.jsonl \
...
/// ```
#[test]
#[ignore = "corpus replay: set TRACE_CAPTURE_DIR and CORPUS_EQUIVALENCE_OUT"]
fn write_corpus_equivalence_fixture() {
    let root = PathBuf::from(std::env::var_os("TRACE_CAPTURE_DIR").expect("set TRACE_CAPTURE_DIR"));
    let out = PathBuf::from(
        std::env::var_os("CORPUS_EQUIVALENCE_OUT").expect("set CORPUS_EQUIVALENCE_OUT"),
    );
    let captures = root.join("captures");
    assert!(captures.is_dir(), "no captures/ under {}", root.display());

    let datasources = DatasourceNames::defaults();
    let settings = AiClassificationSettings::at(true, RECEIVE_SECS);
    let mut file = std::io::BufWriter::new(File::create(&out).expect("create corpus fixture"));
    let mut directories: Vec<PathBuf> = std::fs::read_dir(&captures)
    ...
    for directory in directories {
        let name = directory
            ...
            .unwrap_or_default();
        let Ok(records) = std::fs::read_to_string(directory.join("records.jsonl")) else {
            continue;
        };
        for (sequence, line) in records.lines().enumerate() {
            if line.trim().is_empty() {
                continue;
            }
            let record: Value = serde_json::from_str(line).expect("record json");
            if record.get("signal").and_then(Value::as_str) != Some("traces") {
                continue;
            }
            let Some(mut body) = record.get("body").cloned().filter(|body| !body.is_null()) else {
                continue;
            };
            crate::otlp_json::normalize(&mut body, "resourceSpans");
            let request: ExportTraceServiceRequest =
            ...
            .expect("encode_traces");
            for frame in &frames {
                let payload = String::from_utf8(frame.payload.clone()).expect("utf8 rows");
                for (index, row_text) in payload.lines().filter(|l| !l.is_empty()).enumerate() {
                    let record = json!({
                        "id": format!("{name}/{sequence}/{index}"),
                        "category": "corpus",
                        "capture": name,
                        "row": row_text,
                    });
                    writeln!(file, "{}", serde_json::to_string(&record).expect("record"))
                        .expect("write row");
                    total += 1;
                }
            }
    ...
    println!("wrote {total} corpus spans to {}", out.display());
}
