...
const read = (...parts: string[]): string =>
  readFileSync(join(agentDir, ...parts), "utf8");

const PROMPT_FILES = {
  "instructions.md": read("instructions.md"),
  "skills/dashboard-builder/SKILL.md": read(
    "skills",
    "dashboard-builder",
    "SKILL.md",
  ),
  "skills/incident-investigation/SKILL.md": read(
    "skills",
...
describe("prompt files", () => {
  ...
  test("no chat-flue web-only syntax remains", () => {
    for (const [file, content] of Object.entries(PROMPT_FILES)) {
      expect(content, `${file} must not use the Flue MCP prefix`).not.toContain(
        "mcp__maple__",
      );
      expect(
        content,
        `${file} must not use inline reference cards`,
      ).not.toContain("<<maple:");
      expect(
        content,
        `${file} must not reference submit_diagnosis (web-only tool)`,
      ).not.toContain("submit_diagnosis");
    }
  });

  test("skills tell the model the short names need the maple__ prefix", () => {
    expect(PROMPT_FILES["skills/dashboard-builder/SKILL.md"]).toContain(
      "maple__",
    );
    expect(PROMPT_FILES["skills/incident-investigation/SKILL.md"]).toContain(
      "maple__",
    );
  });
