...
const eveRoot = new URL(
  ".",
  `file://${require_.resolve("eve/package.json")}`,
).href;

const frameworkTools = (await import(
  `${eveRoot}dist/src/runtime/framework-tools/index.js`
)) as {
  getAllFrameworkToolNames(): Set<string>;
  getFrameworkToolDefinitions(input: {
...
const resolvedToolNames = new Set<string>([
  ...REGISTERED_FRAMEWORK_TOOL_NAMES.filter(
    (name) => !authored.has(name) && !disabled.has(name),
  ),
  ...authored,
]);
...
describe("framework tool lockdown", () => {
  ...
  test("the dangerous defaults are gone from the resolved tool set", () => {
    for (const name of [
      ...
      expect(
        resolvedToolNames,
        `${name} must not survive tool resolution`,
      ).not.toContain(name);
    }
  });
  ...
  test("authored tools keep the names the prompts call them by", () => {
    ...
    const instructions = readFileSync(
      join(import.meta.dir, "..", "instructions.md"),
      "utf8",
    );
    const referenced = [
      ...instructions.matchAll(/`([a-z][a-z0-9_]*)` tool/gu),
    ].map((match) => match[1]!);
    expect(referenced.length).toBeGreaterThan(0);
    for (const name of referenced) {
      expect(
        resolvedToolNames,
        `instructions.md tells the model to call \`${name}\``,
      ).toContain(name);
    }
  });
});
