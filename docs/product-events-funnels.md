...
(managed orgs, via `tinybird deploy`) and the frozen copy inside ClickHouse migration 0024 (BYO
clusters). The migration's copy is deliberately NOT imported from the constant: a delta migration
describes one step in history, and a shared constant would silently rewrite what 0024 did the next
...
`product_events` gained `TraceId`/`SpanId` (migration 0024, `DEFAULT ''`, appended, plus a
