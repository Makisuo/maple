# Agentic Journeys — Overview Page Design Brief

Companion to [`agentic-journeys-design-brief.md`](agentic-journeys-design-brief.md), which covers the
journey **detail** view. This brief covers the **overview / list** view that precedes it.

Read the detail brief first — this page must share its visual language, and several of its concepts
(what a journey is, the redacted state, the multi-model problem) recur here in a tighter space.

## Context

A **journey** is one end-to-end agent conversation, reconstructed from OpenTelemetry spans. This page
is how someone finds the journey they care about: a filterable, time-scoped list.
...
"who, from where, how long, how many errors." A journey row has to answer a different question, and
...
**1. What is a journey's title?** The natural label is the first user message — and that is exactly
what privacy modes strip. The identity treatment needs a fallback ladder that degrades to agent name,
then model, then a journey identifier, without any row ever looking empty or broken.

**2. Multiple models in one row.** A journey can hop between models mid-conversation. The detail
...
unusually expensive journey obvious.

**4. Redacted rows.** As in the detail view, message content is frequently unavailable while all
timing, model, token, and cost data remains intact. A list of redacted journeys must still be
scannable and useful, not a column of placeholders.

## Design deliberately: the facets

The replay page's dimensions (browser, OS, device type, country) do not transfer. Journey facets are
...
- Whether the journey used tools
...
The replay page has no sort control. Journeys probably wants one — "most expensive", "slowest", "most
...
- Loading (skeleton)
- Error
- Empty — no journeys in the selected time range, distinct from:
- Empty — filters exclude everything, which needs a path back
- Results capped
- Rows with in-progress journeys mixed into the list
