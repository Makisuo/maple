# Agentic Journeys — Design Brief

Brief for a visual design pass. Scope is the **journey detail view** only; the list/index view is
...
A **journey** is one end-to-end agent conversation: the user's messages, the model's replies, and
...
- Model(s) used — **handle the multi-model case**: a journey can hop between models mid-conversation,
  so the "model" slot needs a graceful representation of two or three
- Total tokens consumed, split into input and output
- Total cost
- Turn count
- Success/error state for the journey as a whole
...
data. Design a first-class degraded state where journey structure, timing, and metrics are fully
intact but message text is unavailable. It must not look broken.

Also:

- Journey still in progress (last message streaming)
- A failed turn where the provider returned an error
- A very long journey (100+ turns) needing navigation
...
**A journey isn't only messages.** The standard defines operations beyond `chat`: tool execution,
...
**Multi-agent handoffs.** One journey can span several distinct agents delegating to each other
...
| Prompt name + version | Ties a bad journey to a specific prompt version. |
