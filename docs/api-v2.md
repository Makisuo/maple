...
Each expected domain error class is created with `HttpTaggedError` and owns its tag, status, stable code, safe copy policy, retry behavior, and recovery action. `publicError(ErrorClass)` derives the exact wire schema from that same definition. A handler only applies `Effect.mapError(toV2Error)` to serialize a domain failure; it does not reinterpret tags with route-local `catchTags` or a second per-domain table. `exposure: "redacted"` requires separate Maple-authored copy at compile time, while the original internal failure remains available for logs and tracing. Boundary-only failures use `defineV2Error` and are emitted through that definition's `make` constructor.
...
4. Handlers in `apps/api/src/routes/v2/<resource>.http.ts`: thin adapters over the existing service — map camelCase/epoch-ms service responses to the wire model and apply `Effect.mapError(toV2Error)`. Register the layer in `ApiV2Routes` (`apps/api/src/runtime/http-graph.ts`).
